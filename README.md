...
```bash
TOKEN=$(curl -s http://127.0.0.1:8848/api/v1/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"email":"admin@careweave.local","password":"ChangeMe-Now-2026!"}' \
...
```bash
curl http://127.0.0.1:8848/api/v1/subjects \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"edition":"elder","name":"王阿姨","birth_year":1946,"profile":{"preferences":["戏曲"]}}'
```

提交机器人说话动作：

```bash
curl http://127.0.0.1:8848/api/v1/actions \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"subject_id":"subj_...","robot_id":"robot_...","action_type":"speak","payload":{"text":"早上好，今天先喝一点水好吗？"},"requested_by":"care_plan"}'
```

动作返回包含风险、理由、义务和`receipt_hash`。若结果为`hold`，监护人可通过审批接口重新评估；硬边界不会因为审批按钮而消失。
...
v1.0.0是**可运行的产品化参考实现和付费试点底座**。核心软件测试已在本交付环境完成；Misty/Furhat网络适配器以及ROS 2、temi、Pepper、Unitree模板需要目标硬件和相应SDK环境完成实机验证。本项目不保证市场主导地位，但通过硬件无关架构、双产品共用内核、监护闭环和证据化安全，设计为具备形成类别领导力的基础。
